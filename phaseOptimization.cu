...
void OptimizationPhase::optimize() {
	...
	while (round < community.graph.edge_destination.size()) {
		...
		auto community_value_pair_output = thrust::make_zip_iterator(thrust::make_tuple(final_community.begin() + save_position, final_value.begin() + save_position));

		auto ne = thrust::reduce_by_key(
			reduced_key_source.begin(),
			reduced_key_source.begin() + n_reduced_edges,
			community_value_pair_input,
			final_node.begin() + save_position,
